...
// Add fonts
// art3m1s extension. Caller must finish GPU readers before resetting the atlas.
int nvgResetFonts(NVGcontext* ctx)
{
	FONSparams params;
	FONScontext* fresh;
	int image, i;
	memset(&params, 0, sizeof(params));
	params.width = params.height = NVG_INIT_FONTIMAGE_SIZE;
	params.flags = FONS_ZERO_TOPLEFT;
	fresh = fonsCreateInternal(&params);
	if (!fresh) return 0;
	image = ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, params.width, params.height, 0, NULL);
	if (!image) { fonsDeleteInternal(fresh); return 0; }
	fonsDeleteInternal(ctx->fs);
	ctx->fs = fresh;
	for (i = 0; i < NVG_MAX_FONTIMAGES; i++) {
		if (ctx->fontImages[i]) nvgDeleteImage(ctx, ctx->fontImages[i]);
		ctx->fontImages[i] = 0;
	}
	ctx->fontImages[0] = image;
	ctx->fontImageIdx = 0;
	for (i = 0; i < ctx->nstates; i++) ctx->states[i].fontId = 0;
	return 1;
}
...
// art3m1s extension. UVs travel with vertices, avoiding a per-rectangle path
// and paint transform. Reflection keeps the winding used by NanoVG text.
int nvgImageQuad(NVGcontext* ctx, int image, float x, float y, float w, float h,
                float u, float v, float uw, float vh, NVGcolor tint)
{
	NVGstate* state = nvg__getState(ctx);
	NVGpaint paint;
	NVGvertex verts[6];
	float corners[8], y0 = y, y1 = y + h, v0 = v, v1 = v + vh;
	if (!ctx->params.renderImageTriangles || image <= 0 || w <= 0 || h <= 0 || uw == 0 || vh == 0)
		return 0;
	if (nvg__isTransformFlipped(state->xform)) {
		float tmp = y0; y0 = y1; y1 = tmp;
		tmp = v0; v0 = v1; v1 = tmp;
	}
	nvgTransformPoint(&corners[0], &corners[1], state->xform, x, y0);
	nvgTransformPoint(&corners[2], &corners[3], state->xform, x + w, y0);
	nvgTransformPoint(&corners[4], &corners[5], state->xform, x + w, y1);
	nvgTransformPoint(&corners[6], &corners[7], state->xform, x, y1);
	nvg__vset(&verts[0], corners[0], corners[1], u, v0);
	nvg__vset(&verts[1], corners[4], corners[5], u + uw, v1);
	nvg__vset(&verts[2], corners[2], corners[3], u + uw, v0);
	nvg__vset(&verts[3], corners[0], corners[1], u, v0);
	nvg__vset(&verts[4], corners[6], corners[7], u, v1);
	nvg__vset(&verts[5], corners[4], corners[5], u + uw, v1);
	memset(&paint, 0, sizeof(paint));
	nvgTransformIdentity(paint.xform);
	paint.extent[0] = paint.extent[1] = 1.0f;
	paint.image = image;
	tint.a *= state->alpha;
	paint.innerColor = paint.outerColor = tint;
	if (!ctx->params.renderImageTriangles(ctx->params.userPtr, &paint, state->compositeOperation,
		&state->scissor, verts, 6, ctx->fringeWidth)) return 0;
	ctx->drawCallCount++;
	ctx->fillTriCount += 2;
	return 1;
}
