...
// art3m1s extension: textured rectangle with explicit normalized UVs and tint.
// Preserves transform, scissor, alpha and compositing; does not change the path.
// Returns zero without drawing when the backend cannot handle this operation.
...
// Local extension: discard all fonts and atlas images between completed frames.
// Invalidates font handles; ordinary image textures and the renderer survive.
...
struct NVGparams {
	...
	// Optional art3m1s extension; vertices and paint must be copied before return.
	int (*renderImageTriangles)(void* uptr, NVGpaint* paint, NVGcompositeOperationState compositeOperation, NVGscissor* scissor, const NVGvertex* verts, int nverts, float fringe);
};
