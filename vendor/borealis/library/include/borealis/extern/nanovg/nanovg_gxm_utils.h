...
// art3m1s: optional conservative completion barrier for hardware flicker A/B.
// Configure outside a frame. Disabled by default; display queue sync remains.
void gxmSetPresentCompletionWait(int enabled);
int gxmSetAsyncPresent(int enabled);
...
struct display_queue_callback_data {
    void *addr;
    SceGxmNotification completion;
};

static int gxm_present_completion_wait = 0;
static volatile unsigned int *gxm_present_notifications = NULL;
static unsigned int gxm_present_serial[DISPLAY_BUFFER_COUNT] = {0};
static SceGxmNotification gxm_present_completed_scene = {NULL, 0};
static NVGXMframebuffer *gxm_active_framebuffer = NULL;
...
static void display_queue_callback(const void *callbackData) {
    ...
    // The queue owns a copy of this notification. Wait on this frame only,
    // without stalling the producer or waiting for later submitted scenes.
    if (cb_data->completion.address)
        GXM_CHECK_VOID(sceGxmNotificationWait(&cb_data->completion));
...
void gxmBeginFrameEx(NVGXMframebuffer *fb, unsigned int flags) {
    gxm_active_framebuffer = fb;
    gxm_present_completed_scene = (SceGxmNotification){NULL, 0};
...
void gxmEndFrame(void) {
    SceGxmNotification completion = {NULL, 0};
    if (gxm_present_completion_wait == 2 && gxm_present_notifications &&
        gxm_internal.window && gxm_active_framebuffer == gxm_internal.window->fb) {
        unsigned int slot = gxm_active_framebuffer->gxm_back_buffer_index;
        completion.address = &gxm_present_notifications[slot];
        completion.value = ++gxm_present_serial[slot];
    }
    GXM_CHECK_VOID(sceGxmEndScene(gxm_internal.context, NULL,
                                completion.address ? &completion : NULL));
    gxm_present_completed_scene = completion;
}

void gxmSetPresentCompletionWait(int enabled) {
    gxm_present_completion_wait = enabled != 0;
}

void gxmSetNativePresent(int enabled) {
    // SHUF00002 0x81031D64: enqueue display, rotate buffers, then Finish.
    gxm_present_completion_wait = enabled ? 3 : 0;
}

int gxmSetAsyncPresent(int enabled) {
    // Configure outside a scene. Drain previous callbacks before resetting
    // sequence counters, including when toggling or recreating a window.
    sceGxmFinish(gxm_internal.context);
    sceGxmDisplayQueueFinish();
    gxm_present_completed_scene = (SceGxmNotification){NULL, 0};
    gxm_present_notifications = enabled ? sceGxmGetNotificationRegion() : NULL;
    gxm_present_completion_wait = enabled ? 1 : 0; // conservative fallback
    if (!gxm_present_notifications) return 0;
    // This NanoVG GXM context owns slots 0..DISPLAY_BUFFER_COUNT-1. No other
    // subsystem in this host uses the GXM notification region.
    for (unsigned int i=0; i<DISPLAY_BUFFER_COUNT; ++i) {
        gxm_present_notifications[i] = 0;
        gxm_present_serial[i] = 0;
    }
    gxm_present_completion_wait = 2;
    return 1;
}
...
void gxmSwapBuffer(void) {
    ...
    NVGXMframebuffer *fb = window->fb;
    if (!fb) return;
    // Called after endScene and common-dialog submission. Ensure rendering is
    // complete before the display request becomes visible. This serializes
    // CPU/GPU work, so enable only in the hardware synchronization candidate.
    if (gxm_present_completion_wait == 1 ||
        (gxm_present_completion_wait == 2 && !gxm_present_completed_scene.address))
        sceGxmFinish(gxm_internal.context);
    ...
    queue_cb_data.completion = gxm_present_completion_wait == 2
        ? gxm_present_completed_scene : (SceGxmNotification){NULL, 0};
    ...
    if (gxm_present_completion_wait == 3) sceGxmFinish(gxm_internal.context);
}
