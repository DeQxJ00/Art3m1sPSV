  ...
  public:
    ...
    inline static void log(LogLevel level, std::string prefix, std::string color, fmt::format_string<Args...> format, Args&&... args)
    {
        ...
        try
        {
            ...
            // The Vita debug channel does not write to setLogOutput's FILE.
            if (logOut && logOut != stdout && logOut != stderr) {
                std::fprintf(logOut, "%02d:%02d:%02d.%03d [%s] %s\n", time_tm.tm_hour, time_tm.tm_min, time_tm.tm_sec, (int)ms, prefix.c_str(), log.c_str());
                std::fflush(logOut);
            }
#elif defined(PS4)
