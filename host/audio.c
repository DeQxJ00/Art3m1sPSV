...
static int open_decoder(Track *t,const char *path){
    int r;
    t->vorbis=host_vorbis_open(path,&t->rate,&t->channels);
    if(t->vorbis){
        if(t->rate!=48000){
...
static void *audio_worker(void *unused){
    ...
    for(;;){
        ...
        pthread_mutex_lock(&mutex);int go=running;Command *cmd=first;first=last=NULL;pthread_mutex_unlock(&mutex);if(!go)break;
        ...
        for(int n=0;n<TRACKS;n++){Track *t=&tracks[n];if(!t->active)continue;
...
int host_audio_start(void){running=1;pthread_attr_t attr;pthread_attr_init(&attr);pthread_attr_setstacksize(&attr,512*1024);int r=pthread_create(&worker,&attr,audio_worker,NULL);pthread_attr_destroy(&attr);started=r==0;return r;}
