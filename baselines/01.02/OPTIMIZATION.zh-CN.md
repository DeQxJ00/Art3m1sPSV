...
已实现显式编译门控的受保护等待路径及延迟消费测试，说明见 `tests/direct_gxm/DEFERRED_FINISH.md`。仅 `direct_probe` 接收 `DIRECT_DEFERRED_FINISH_PROBE`；默认 optK 宿主 GPU 目标文件重编译后 SHA256 与原来一致，仍为 `01075f1feff40f257b61d4f00d323bfa5ebc72e25580f38ea9cbf3d518a19afd`。
...
## 后续本地候选：不可变历史页快照缓存（未替换 optL）

源代码审计发现 `Backlog` 的历史页数组为私有，只有 push_page/clear 能改存储，page() 仅返回共享引用；但 `BacklogInputs::update` 每次同步仍逐页深比较字体参数和标签。历史达到上限发生前移后，按下标比较还会使所有剩余页失配并重新序列化。这是明确可去掉的重复工作；尚不能据此把实机 backlog+文本度量共计约 4.5 ms 全归到历史比较。

`Backlog` 现在保存 `Arc<BacklogPage>`，不改变公开 page() 的借用返回类型或序列化输出。有效写入/清空更新独立 `Arc<()>` 修订身份；配置变更或被拒绝的空页/禁止写入不改变现存历史。缓存持有旧身份直到更新，避免计数回绕、实例替换或分配器地址复用产生误命中。

历史未变时比较修订身份后直接复用；变化时按仍被持有的不可变页身份移动已有序列化结果，只生成新页标签。这样满页滚动保留旧页字符串分配，也不再深拷贝整份历史输入。当前消息层仍可被脚本和翻译直接修改，因此继续比较它的实际字体/标签内容；没有用同样的快捷路径跳过可变消息层。没有减少历史页数、缩短文字、改变字体/效果或改 shader。

语义测试把缓存输出与原始完整 build_backlog_snapshot 比较，覆盖中文、引号/反斜杠、注音、换行、满 100 页滚动、同页数的新历史实例、仅清空输出、缩小容量、include_font=false、清空及旧页释放。存活页滚动后序列化向量地址不变，Weak 检查确认已淘汰页不被缓存长期保留。原有当前消息同长度文字/字体修改回归继续通过。

`build/01.02-history-cache/benchmark.log` 的 release CPU 微基准，当前消息层留空以隔离历史同步，并用改动前的历史算法与相同 live-layer 更新路径作对照：

| 历史场景 | 原算法 | 候选 |
| --- | --- | --- |
| 0 页，重复 20000 次，平均/次 | 0.009 µs | 0.006 µs |
| 10 页，重复 20000 次，平均/次 | 3.718 µs | 0.006 µs |
| 100 页，重复 20000 次，平均/次 | 42.314 µs | 0.006 µs |
| 100 页满容量，新增并滚动 1000 次，平均/次 | 830.250 µs | 12.306 µs |

这些是桌面 CPU 数据，**不是实机整帧或换句耗时**，也没有包含实际消息层度量和 GPU 工作；不能据此宣称已消除实机换句低帧。
...
本轮以 optP 证实的“动画持续触发整场景重建”为依据，减少每次重建中重复的排序和临时数组分配。图层属性、动画求值及绘制内容照常更新；仅复用结构未变的子树顺序。实现、失效边界和测试说明见 `tests/direct_gxm/SCENE_ORDER_CACHE.md`。命中路径使用 OnceLock 已初始化读取，不恢复之前每个子树都持有 Mutex 的方案；冷初始化仍可能同步。

局部缓存、公开 children 可变访问失效、删除释放、重命名、别名稳定顺序与存档往返均通过测试。新增 160 轮完整 DrawList 对照；全套 test-all 962 项通过、23 项忽略、1 项既有 pf8 路径失败，pfs-upk 另 6 项通过。528 节点、每次改变父层旋转的桌面整场景基准，两组关闭/开启分别 174181/64428、121359/57768 µs（1000 次构建），减少约 63% 和 52%，不能当作 PSV FPS。

MCP 先检查旧会话状态，随后会话 `32a5c9ac-2354-4c05-a2e8-0320d570a668` 运行独立 optQ 包。开篇“如果云层之上真有神的国度……”两行正文，保持场景不操作，排序缓存开／关／开各 30 秒；计数仍由合成器动画持续触发重建。三个阶段均 273 quad / 22 draw / 1 uniform，444/222/222/166 时钟未改，语音 0、一条音轨，无解码/上传。

| 最后一份完整 core 窗口，ms/绘制 | 开，首段 | 关 | 开，恢复段 |
| --- | --- | --- | --- |
| 场景构建 | 0.3184 | 0.4873 | 0.2952 |
| 全部帧构建 | 0.6675 | 0.8575 | 0.6317 |
| 当前消息同步 | 0.0942 | 0.0993 | 0.0918 |

场景构建分项减少约 35%～39%，全部帧构建减少约 22%～26%。宿主含等待总耗时三个阶段都是约 16.66 ms；over20ms 分别 77/3/1，窗口帧数分别 1805/1203/1203，不能宣称本轮消除了瞬时卡顿。日志中的每窗口 reads=301/missing=301 在旧 optO 的相同场景也存在，未因本轮排序改动产生；具体缺失资源未定位，下一轮可继续核查，不能直接负缓存所有缺失资源而破坏动态创建。

三个阶段正文区域 `[220,425,884,485]` 逐像素一致；Backlog 三块正文开关前后也完全一致，缓存恢复后向前翻页正常。`build/01.02-optQ/emulator-ab/` 保留原始日志、manifest、comparison、interpretation、截图与像素比较；构建/回归日志和缓存测试说明单独保留。没有改变 shader 或 GPU 目标文件（仍为 optK 的 SHA256）。
