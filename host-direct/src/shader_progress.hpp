...
enum class ShaderStage : int { Read=0, Ready=1, Failed=2, Builtin=3, Source=4, Cache=5, Compile=6, CacheHit=7 };
...
struct ShaderProgress {
    ...
    bool should_present(uint64_t now,uint64_t previous,bool first)const{
        return seen&&(first||stage==ShaderStage::Compile||finished()||now-previous>=100000);
    }
};
}
