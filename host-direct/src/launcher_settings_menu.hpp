...
struct LauncherSettingsMenu {
    ...
    void prepare()const{
        for(auto s:{"启动器设置","CapUnlocker","超频设置","返回游戏选择","开","关",
            ...
            "转换：将支持的 HLSL 转成 Cg，下次进入游戏生效。",
            "编译：将 Cg 编成 GXP，下次进入游戏生效。",
            "关闭后仍可使用内置效果和匹配的已有缓存。",
    ...
    void draw(const Cpu3Setting& cpu3,const ShaderSettings& shader)const{
        ...
        else if(row==2||row==3){menu_text(160,442,24,row==2?"转换：将支持的 HLSL 转成 Cg，下次进入游戏生效。":"编译：将 Cg 编成 GXP，下次进入游戏生效。");
            menu_text(160,476,24,"关闭后仍可使用内置效果和匹配的已有缓存。");}
