...
int art3m1s_gxm_upload_alpha_region(uint64_t id,uint32_t w,uint32_t h,const uint8_t* p,size_t length,
    uint32_t x,uint32_t y,uint32_t rw,uint32_t rh){
    if(!direct::alpha_texture_allowed())return -1;
    if(!p||length!=uint64_t(w)*h||!rw||!rh||x>=w||y>=h||rw>w-x||rh>h-y)return 0;
    auto* t=find(id);
    if(t&&t->alphaOnly&&t->w==w&&t->h==h){
        if(!direct::update_alpha(t,p,x,y,rw,rh))return 0;
        t->contentRevision=++textureRevision;return 1;
    }
    auto* next=direct::texture_alpha(w,h,p);if(!next)return 0;
    next->contentRevision=++textureRevision;textures[id]=next;direct::destroy(t);return 1;
}
