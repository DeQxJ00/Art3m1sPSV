...
bool gpuRequested=false;unsigned gpuCaptureW=0,gpuCaptureH=0;direct::Texture* gpuCapture=nullptr;
...
int art3m1s_gxm_capture_previous_texture(uint64_t id,uint32_t w,uint32_t h){
    if(!w||!h)return 0;
    if(gpuCapture&&w==gpuCaptureW&&h==gpuCaptureH){
        auto* old=find(id);textures[id]=gpuCapture;gpuCapture=nullptr;direct::destroy(old);
        direct::log("[transition-snapshot] bound id=%llu logical=%ux%u",(unsigned long long)id,w,h);return 1;
    }
    if(gpuCapture){direct::destroy(gpuCapture);gpuCapture=nullptr;}
    gpuCaptureW=w;gpuCaptureH=h;gpuRequested=true;return 0;
}
void art3m1s_gxm_finish_host_frame(){
    if(gpuRequested){gpuCapture=direct::snapshot_completed();gpuRequested=false;}
    if(requested){capture.resize(size_t(captureW)*captureH*4);
    if(!direct::readback(captureW,captureH,capture.data()))capture.clear();requested=false;}}
void art3m1s_gxm_cancel_capture(){requested=false;capture.clear();gpuRequested=false;
    if(gpuCapture){direct::destroy(gpuCapture);gpuCapture=nullptr;}}
