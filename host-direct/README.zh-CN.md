...
游戏选择界面的 START 设置提供 Debug 调试开关，默认关闭，已有开启设置会保留，修改后重启应用生效。关闭时仍执行渲染能力像素校验，但不把测试画面送到屏幕；开启时显示原有开篇自检过程。两种模式使用相同的校验结果来启用缓存、共享纹理和单通道字形优化。设置保存于 ux0:data/art3m1s-gxm/debug-settings.txt。
