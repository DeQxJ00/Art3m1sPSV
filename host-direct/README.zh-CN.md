...
VPK 内置 **内置 Shader 演示 demo**，在游戏选择界面直接进入；51 个来源文件按完整字节去重为 31 项，相同 shader 仅保留一份源码和一个演示页。每页标注 game1/game2 来源，manifest 保留全部来源映射。按 ○ 下一项、31 项后循环，按 □ 呼出菜单退出。无需复制 demo 或开启自动 shader 转换/编译。
