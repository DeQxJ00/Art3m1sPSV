...
regions=('heap','cdram','uncached');owners=('source','decode','ready','provider','texture','fixed','offscreen','temporary','media','font')
...
for number,line in enumerate(a.log.read_text(encoding='utf-8',errors='replace').splitlines(),1):
    ...
    raw.setdefault('media','0');raw.setdefault('font','0') # Accept the preceding A1 snapshot format.
...
result={'log':str(a.log),'complete_snapshots':len(snapshots),'errors':errors,'last':snapshots[-1] if snapshots else None,'coverage':'Image CPU paths and Direct-owned GPU blocks only; Explicit OGV/font buffers included by newer hosts; audio/codec/driver/scratch not fully tracked.'}
