...
& "$rustPath/bin/cargo.exe" rustc --manifest-path "$PSScriptRoot/../core/Cargo.toml" --lib --release --no-default-features -Z build-std=std,panic_abort --target armv7-sony-vita-newlibeabihf --crate-type staticlib --features gl-backend,gxm-native-renderer,gxm-text-epoch,gxm-menu-key-alias,gxm-builtin-effects
