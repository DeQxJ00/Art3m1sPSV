...
New-Item -ItemType Directory -Force "$root/build/video-yuva-gxm" | Out-Null
...
& "$rustPath/bin/cargo.exe" rustc --manifest-path "$root/core/Cargo.toml" --lib --release --no-default-features -Z build-std=std,panic_abort --target armv7-sony-vita-newlibeabihf --crate-type staticlib --features gl-backend,gxm-native-renderer,gxm-text-epoch,gxm-menu-key-alias,gxm-builtin-effects
if($LASTEXITCODE -ne 0){throw 'Core build failed'}
Copy-Item -LiteralPath "$env:CARGO_TARGET_DIR/armv7-sony-vita-newlibeabihf/release/libart3m1s_core.a" -Destination "$root/build/video-yuva-gxm/libart3m1s_core.a"
