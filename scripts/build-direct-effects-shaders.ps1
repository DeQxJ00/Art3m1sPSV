...
$out=Join-Path $root 'build/shader-completion/shaders'
...
foreach($shader in @('sprite_v','sprite_f','image_f','clip_f','rule_f','ruleclip_f','builtin_f')) {
  ...
  & $compiler -profile $profile -O1 -nofastmath -bestprecision -o $binary (Join-Path $root "host-direct-effects/shaders/$shader.cg")
...
[IO.File]::WriteAllLines((Join-Path $root 'host-direct-effects/src/shaders.hpp'),$lines)
