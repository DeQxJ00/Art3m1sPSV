...
int main(){
    ...
    sceIoRemove("ux0:data/art3m1s-gxm/host.effects.previous.log");sceIoRename("ux0:data/art3m1s-gxm/host.effects.log","ux0:data/art3m1s-gxm/host.effects.previous.log");
    output=std::fopen("ux0:data/art3m1s-gxm/host.effects.log","w");if(output)std::setvbuf(output,nullptr,_IOFBF,32768);
    direct::log("Direct GXM built-in effects build %s %s; gray/negative, 10 blends, masks/groups/native emote",__DATE__,__TIME__);if(output)std::fflush(output);
    ...
    const char* title="art3m1s  /  GXM Effects";const char* help="○ 确认   × 退出   ↑↓ 选择";
    for(;;){
