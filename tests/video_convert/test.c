...
static void levels(void) {
    ...
    printf("legal YUV cube RGB max_error=%d/255; all 256 alpha levels exact\n",max);
}
static void recording(const char *color,const char *mask) {
    ...
    assert(s&&g);int frames=0,max=0;unsigned long long sum=0,count=0;
    ...
    printf("recording frames=%d RGB max_error=%d mean_error=%.6f alpha exact\n",frames,max,(double)sum/count);
    assert(frames>0&&max<=2);sws_freeContext(s);sws_freeContext(g);free(p);fclose(fc);fclose(fm);
}
int main(int argc,char **argv){borders();levels();if(argc==3)recording(argv[1],argv[2]);return 0;}
