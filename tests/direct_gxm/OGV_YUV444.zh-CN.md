...
用户确认第八轮 Artemis 阶段是卡住，并非主动切后台。现场完整日志与旧日志
保存在 build/hardware-logs/20260912-190633-companion；此包未链接 uvdb，
无法对这次现场直接附加取栈。未改包，保存现场后 kill/launch 重试。
重启后正常进入樱花，不能据此宣称卡住已修复；原因仍待复现定位。

实机缓存终于取得证据：candidate8-relaunch-play.log 中 color_bytes=1375976、
mask_bytes=487939；相邻窗口 color_hits 44→60→73、mask_hits 15→21→26，
color_disk_reads/mask_disk_reads 始终 4/4。两路累计服务量超过原文件大小，
循环期间未增加磁盘读取。333 MHz、Q6 oracle 通过；五个视频窗口输出
55/5015、53/5004、53/5082、53/5045、54/5078（帧/毫秒），仍约 10–11 fps。
缓存生效不等于实时 Theora 解码/转换成本被消除。

后续转换方案仅讨论，尚未替换素材或实现新播放格式：H.264 颜色硬解，
配对遮罩离线生成 A8 帧并快速压缩；若继续沿用 CPU RGBA 合并，仍有整帧
复制、写 alpha、上传成本。进一步方案是独立视频 GXM 颜色/透明度合成路径，
保留现有游戏 shader；性能、边缘透明度和时间戳仍需专门验收。
