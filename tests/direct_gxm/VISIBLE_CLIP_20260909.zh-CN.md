...
状态（18:12 更新）：固定双人页模拟器视觉 A/B 与实机 ON/OFF/ON 均完成。当前页实机开启约 44 FPS，关闭约 30 FPS；收益明确，尚未达到 60 FPS。
...
## 实机固定页 ON/OFF/ON（18:10–18:12）

用户确认页面就绪后，通过 VitaCompanion 健康检查，再仅创建/移除候选的 `visible-clip.off` 文件。没有按键输入、重启、频率调整或资源修改。每段等待 30 秒后取日志；取各段末尾三个完整稳定窗口，排除切换混合窗口，按窗口帧数加权。统计为程序记录的帧循环耗时推算 FPS，不是外部显示面板读数。

| 状态 | 帧耗时 | 推算 FPS | 帧首 GPU 等待 | 逻辑耗时 |
| --- | ---: | ---: | ---: | ---: |
| ON1 | 22.410 ms | 44.623 | 6.575 ms | 5.900 ms |
| OFF | 33.864 ms | 29.530 | 17.302 ms | 6.102 ms |
| ON2 | 22.896 ms | 43.675 | 6.479 ms | 5.994 ms |

三段均为 96 quads、30 draws，requested clip=5；OFF uniforms=5，ON uniforms=0。每段分别采用 667、443、656 帧。两次 ON 的平均帧耗时相对 OFF 降低约 33%，FPS 提高约 49%。这是同场景、同程序的选择路径对照，不能拿旧的不同人物页 41–43 FPS 判断此改动无效。由于移除裁剪也可能使已有的不透明快路径适用，收益应归于整个冗余裁剪选择改动，不能精确分解成每条 shader 指令耗时。

证据：`build/hardware-clip-ab/20260909-181036/`，包括原始日志、阶段时间及哈希、恢复记录、`analysis.json`；采样器 `build/heap-audit/clip-hardware-ab.py`，分析器 `build/heap-audit/analyze-clip-ab.py`。结束时再次检查远程标记不存在，优化保持开启。

下一项应分析剩余约 22.7 ms 的构成及立绘/头像透明区域的重复绘制；不能因为本项有收益就删除 GPU 安全等待或宣称达到原生 60 FPS。维持 shader 字节码不变。
