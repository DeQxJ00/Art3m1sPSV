...
2026-09-15 补充：otomeriron PC EXE 原语探针确认脚本模式 2 的灰度图层不会
把周围背景覆盖成黑色；旧 core 的 mode-2 强制不透明假设已撤销。具体证据与
修正范围见 [回想透明覆盖调查](OTOMERIRON_GRAY_COVERAGE.zh-CN.md)。这项结果
不等于模式 1/2 的所有生命周期差异都已还原，也不代替上述 PSV 样本的逐项验证。
...
Reclaim diagnostic 9eed162 deployed via `build/direct-deploy/deploy-20260910-065107/manifest.json`; installed eboot SHA256 f85a7a267fc8fb3f7483fc058a850219799a1b7a70ee711412438a0bcefd5ad9. Startup `build/hardware-logs/20260910-065310-current/host.log`: retained self test PASS, 333/222/111/111 MHz. Actual isolated gpu.cpp/bridge.cpp/main.cpp normalized contents match host 9d17ae2. Awaiting reproduction of transition latency; latest stable package still points to accepted e369f5e, diagnostic package is separately immutable under `build/direct-candidates/cache-reclaim-9eed162`.

### 2026-09-10: ready-pixel retention budget experiment (not accepted)

Reproduction on 9eed162: `065517-current/first-use-decode-result.json` shows zbg27k and tor_z2a0100 first use consumes encoded prefetch payloads; provider eviction diagnostic was not exhausted and had no prior eviction of those assets. zbg27k decode 297821us + upload 105109us coincides with a 446570us max frame window. A distinct later-repeat path also includes disk rereads; do not conflate these.

Core e1a2c8b trials 96MiB ready payload retention, leaving 16MiB single-source/decode allocation limits, one priority-180 worker, queue bound 64, promotion/wait/cancel/take protocol unchanged. No rejected redecoding or second-worker extension is restored. The observed chapter readiness announcements sum to about 76.3MiB (not simultaneous live bytes); UI and earlier ready data add about 24.8MiB. This motivates a measurable capacity experiment, not a native-derived magic number. Log now reports actual retained payload kind/bytes and current total cache charge. Extra ready capacity is up to 80MiB; no claim of bounded total process peak follows from that alone. Hardware memory and fast-skip validation required. 396 native tests pass / 13 ignored, including a scaled case proving budget pressure drops first-use pixels at 160 bytes but keeps them at 256 without changing the protocol. Production candidate not yet deployed; stable latest remains accepted e369f5e.

96MiB trial e1a2c8b deployed via `build/direct-deploy/deploy-20260910-070041/manifest.json`; installed eboot SHA256 ad8e513511ef740f2fa26e5afd5e462401dfdaf38109d267765b9a9cd4c4891e. Startup `070121-current/host.log` retained self test PASS and 333/222/111/111 MHz. This is startup validation only; user is asked to reproduce first-use background/portrait transitions and a short fast-skip sequence, while logging real ready payload kind and heap usage. Stable latest package remains e369f5e. The offline summary tool now includes ready-cache publication samples, heap samples and bounded reclamation records without interpreting sampled heap usage as a total peak guarantee.
