...
按用户最终指定，开关直接放在启动后的游戏选择界面底部，不加入游戏内菜单，也不放进字号设置。按 △ 或触摸 `(36,466)–(924,500)` 一行直接切换，成功保存后更新开／关显示。设置对所有游戏生效；必须关闭并重新启动应用，现有线程和本次启动后创建的线程都继续遵循原启动策略。不会自动重启程序或安装／改动内核插件。

继续以 `ux0:data/art3m1s-gxm/cpu3.off` 为唯一配置：存在表示关，不存在表示开。保存调用创建／移除标记后回读确认；读写失败显示错误，不把普通 I/O 错误当作开启授权。只在启动读取和用户切换时访问文件，不加入游戏帧循环或音频线程轮询。`[cpu3-menu]` 日志包含 next_start、current_request 与保存结果，可区分下次配置和当前实际请求。

本轮只修改宿主的菜单与配置接口，核心仍为 `d015d02`，shader、字体覆盖和调度策略不变。Vita VPK 构建通过；使用实际 stb 字形验证游戏选择页新增行与底部提示的边界，960×544 预览无重叠。启动、切换持久化和重启后的策略日志待实机验证。

2026-09-11：用户明确确认“CapUnlocker 第四核心验收成功”，将支持项标记完成，并按要求从 v1.2.2 增加 0.0.1 为 `v1.2.3`。这次同时修正备用总菜单中“字号设置”误用 28px 标题图片的问题，另烘焙 24px 菜单项，与其他操作一致。菜单图集仍为 512 KiB，增加到 33 个区域，内嵌 RLE 46,176 字节；页面标题保持独立标题样式。按此前版本管理约定使用 Git 标签，SFO APP_VER 保持 01.10。

`v1.2.3` 标签指向宿主 `c82159aa844d223db0d57e2f4f0f09f7e08b1436`，`v1.2.3-core` 指向 `d015d0230dd6f32e9c0147f059f8d69ef810705b`。冻结包 `build/direct-candidates/v1.2.3/art3m1s_direct.vpk` 的 SHA256 为 `bd0edfe71dc46771f9e42a2de0238c6353176426829104c77fa8da101136deff`，SELF 为 `9a8d6fea499f6c4180f9213566fd7ca25e08eb4e2f33aa5962758f2030525513`；已通过源码快照、核心静态库未变、VPK 内 SELF 校验。

最终包已上传实机并完成 SELF/SFO 回读校验：`build/direct-deploy/deploy-20260911-101454/manifest.json`。VitaCompanion 返回 Killed/Launched，但随后两次抓取的日志与安装前备份完全相同，SHA256 为 `1d0126db924af5c61b9d49db6c8df3ad356fd6085caadfa7b96d19e59fad5b88`，开头仍为 09:57:10 构建。因此不将其中的启动自检通过计为最终包的运行证据；已请求用户手动重新打开应用确认。第四核心功能本身的用户验收结论仍有效。
