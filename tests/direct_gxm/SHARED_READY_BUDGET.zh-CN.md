...
## 80MiB复测：多保住人物，但大背景仍被降级

只读复制日志build/hardware-logs/20260911-033148-current/host.log，SHA256 366da2c442bfc579bc61ac2e939c96022a68b59fe7187924fdc7770e8b1f0a30。第548行确认shared_budget=83886080；同进程启动自检通过、333MHz。109份缓存快照无errors，76份资源账本无errors/faults；ready峰值71692820字节。heap账本峰值113660645字节（约108.4MiB），CDRAM峰值90963968（86.75MiB），uncached峰值0；这是已覆盖资源的账本峰值，不是整个进程内存或全部场景安全保证。

tor_z2a0100这次Pixels命中，kun和line21..23也Pixels命中；80MiB确实能多保留一部分解码结果。但zbg27k第2754行已经Pixels就绪，第2911/2912行先后降级bg27k与zbg27k，然后发布ev_lth_01d（2287210字节），仍是按旧ticket优先降级背景。第3125行首用Encoded重解码310135us/publish1313us，第3127行整帧357662us；第3948行provider压缩回退重解码304679us/publish1207us，第3949行整帧358437us。背景解码缓存保留目标未达成。

本轮kun+放射转场第4183行416189us（logic135331/present280792）；附近kun的PNG注释读取51551us、人物上传59556us、wipe_13现场read79237us/decode53580us/publish25328us。该帧没有再次背景解码，不能与64MiB后两次背景Pixels加遮罩Encoded的281/290ms简单视为同条件容量退化。流程次数、冷热状态不同，也不能用80MiB全日志2次背景Encoded对64MiB5次来计算命中改善比例。最后完整frame-perf为300帧、max17448us、over20ms=0，仅说明停留窗口，不证明换句无波动。

本轮未改程序、未重启或发按键。继续增加容量没有消除后续预载挤掉先用背景的问题；后续宜以实际消费优先级和淘汰/准入规则为主，并分别处理首次GPU上传、转场资源准备与纯文字换句重建。当前80MiB包保留供用户观察，64MiB包仍可回退，尚未把80MiB评为普遍优于64MiB。
