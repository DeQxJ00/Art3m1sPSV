...
用户暂不采用像素格式变化，要求回到扩大缓存。core 1510510仅把session保留总额48MiB改为64MiB（67108864字节）；沿用5/6请求比例，pending时ready_goal=55924054，剩余11184810字节供idle使用。闲置上限32MiB、压缩源上限4MiB、单次解码工作额度16MiB、淘汰顺序及线程/取消/GPU协议均不变。worker启动日志改用同一容量常量，避免诊断数字与实际额度脱节。
...
## 64MiB部署完成，启动自检异常仍待定位

用户恢复连接后，deploy-20260911-030700完成旧48MiB文件备份、新64MiB文件上传和读回校验，launch成功；该次kill返回cannot kill，未据此声称先前游戏在运行。新启动日志030736-current（SHA256 3a299d50c479fd3cf95c1ee056851bcfaf66aa1cb7aa3afb672a6e1eeaa702db）shared自检max_delta=255、ok=0；保留该日志后对同包仅重启一次，030830-current（1365b3d877c5baedfb32fa6a86bbda37154a6e17e02bd7e56ff62ff011e09530）仍max_delta=255、ok=0。这两轮共享路径均被禁用，不作为容量性能对照。
...
用户确认关闭性能浮窗后，health→kill→launch重启同一80MiB包。第一次日志032502-current仅启动首行，未当作成功；随后032542-current（SHA256 cd26ce9c10b6b782cde4dae5b4c376d8787d524e3ff515b736d78f50b016d93f）shared max_delta=0、ok=1，其他启动自检通过、333MHz。结合之前差异仅落在左上角小区域，这次对照支持浮窗更新干扰整屏比较的解释，但未修改测试以抵御外部覆盖，尚不能宣称之后带浮窗启动均可通过。当前进程共享路径已启用，可恢复浮窗并进行80MiB游戏缓存复测；仍需进入游戏后确认83886080运行预算和目标资源命中。
