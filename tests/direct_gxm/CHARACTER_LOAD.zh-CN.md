...
用户回复“好了”。00:13只读日志SHA256 `35f5871a2110e1a4cc11a7dacf9c18aa5b784d1677847573116f41dfd30492f6`，出现缓存hits=1/2/3/4/64，均read_bytes=0，覆盖koh头像和tor立绘。kun_z2a0100首次注释查询39180μs，上一版同资源74627–116883μs；这是不同运行的单资源观测，不是受控FPS A/B。缓存命中路径有效，首次查询仍可能因存储竞争达数十毫秒，未声称完全消除长帧。

287.173秒处整帧1452049μs，present1340124μs：相邻zbg27k重新读429348μs、解码309512μs、上传约94501μs，人物解码82223μs／上传约52146μs，转场规则wipe_13读186496μs／解码55648μs；此外还有动画帧准备，嵌套分项不得直接当互斥总和相加。图片/规则缓存衔接仍未修复。之后5个300帧窗口over20ms=0、最大17.3–17.6ms，但最后仍有88755μs的present主导单帧，原因未定，不能宣称停句全程无卡顿。

55组完整账本快照faults=0，末尾heap已记录52,188,593字节、CDRAM67,371,008字节、audio35008、media0；newlib实际used69,501,032，不把未记录差额误认为全部可用于扩缓存。未退出菜单，不作为全部资源释放验收。日志还有两次逻辑主导的12.58s/10.67s长帧，未确认是否包含系统前后台/菜单操作，暂不归因于注释或图片缓存。

后续缓存改动需同时解决首次使用前像素被降级、再次使用时源数据已耗尽、转场规则冷读取；新增压缩源层若直接占用固定份额，也可能挤掉原本可命中的像素缓存，不能只看避免读盘的收益。保持总额度与异步协议，先验证所有权/冷热淘汰及动画回归，再发布实机候选。

[日志](evidence/character-load-20260911/host.log)、[摘要](evidence/character-load-20260911/summary.json)、[账本](evidence/character-load-20260911/ledger.json)。本轮未替换用户正在运行的程序。
