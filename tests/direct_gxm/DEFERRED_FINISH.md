...
## 2026-09-09 14:52 实机同画面对比

用户确认已停在正文，要求保持长句、语音结束。VitaCompanion 读取恢复正常；实际运行 optL，状态日志确认 ARM 333 / bus 222 / GPU 111 / xbar 111 MHz。没有安装新包或发送翻页输入。

`build/profile-ab/20260909-145255/` 保存原始 on/off/on 日志、manifest 和 comparison。临时控制文件最终恢复原先不存在的状态（帧首等待开启）。三个阶段的绘制负载均为 405 quads / 29 draws / 2 uniforms。

| 阶段 | 宿主分项合计平均帧时 | 等待平均耗时 |
| --- | --- | --- |
| 帧首 A | 25.806 ms | 7.135 ms |
| 帧末 | 30.493 ms | 11.667 ms |
| 帧首 B | 25.973 ms | 7.148 ms |

两次帧首结果接近；相对帧末每帧减少约 4.5–4.7 ms（平均帧时约低 15%，吞吐约高 18%）。等效平均速率约 38.5–38.8 对 32.8 FPS，尚未达到 60。数值由宿主窗口的分项均值相加得到，不是屏幕逐帧采集的 p99。首段提取含首次 FTP 读取之后的完整窗口，可能覆盖采样启动前的连接间隙；后两段已排除模式切换边界。

静止正文日志仍有约 12 ms/render 的 CPU 构建，语音活动提示为 0、保留 1 条音轨；稳态没有纹理上传。这个结果支持保留受保护的帧首等待候选，并继续检查 CPU 构建。它不能证明换句峰值已解决，也不能替代用户对闪烁/缺块的观察。后续 CPU 候选尚未在这轮装入，不将该收益算给 optQ/optR。
