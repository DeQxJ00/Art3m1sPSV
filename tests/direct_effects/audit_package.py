...
bad=re.compile(r'brls::|\bnvg[A-Z]|\bnvgxm|nanovg|borealis',re.I)
...
assert all(not re.search(r'borealis|nanovg',p.read_text(),re.I) for p in deps)
...
with zipfile.ZipFile(vpk) as z:
    ...
    assert all(not re.search(r'borealis|nanovg',p,re.I) for p in z.namelist())
