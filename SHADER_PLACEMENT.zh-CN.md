...
最新版引擎已内置 otomeriron、Toshiue_Kanojo2 的 51 个来源文件对应的 31 种独立效果。游戏原来的 HLSL 文件仍需保留在资源包或原资源目录中，程序按源码内容匹配内置效果；匹配成功时无需额外 CG/GXP，也不会为这些效果生成编译缓存。
...
保留原文件后缀：`example.hlsl.cg` 不能改成 `example.cg`。缓存放在各自游戏目录下，不放在 VPK 的应用目录、所有游戏共用的缓存目录或原 PFS 内。
...
├─ assets/
└─ sources/
   ├─ otomeriron/system/shader/pc/       ← 20 个原始 HLSL
   └─ toshiue/system/shader/pc/          ← 31 个原始 HLSL
```

整个 `TEST_SHADERS_51` 文件夹原样复制即可。demo 为区分两个来源，脚本明确使用上述 `sources/...` 路径，不要把里面的 HLSL 移到其他位置。51 项均匹配内置效果，保持两个自动开关关闭即可，不必新建 `shader-cache`。

如果将来修改 demo 的源码做额外转换测试，其缓存同样保留完整相对路径，例如：

```text
TEST_SHADERS_51/shader-cache/sources/toshiue/system/shader/pc/example.hlsl.cg
```
