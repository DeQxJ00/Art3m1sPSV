...
另已内置 Stella PC／Android 共用的 `blend`、`blend2` 和 `radial` 源码版本，内置识别共 34 项。两份资源只存一份实现；Stella 的 `radial` 使用其已通过实机验证的预编译版本，保留原有 `radial` 识别。上述三项也不依赖自动转换／编译开关或 shader-cache。
